...
// --- Sun Slideshow ---
const selfieImages = [
  "./asset/Mai1.jpeg","./asset/Mai2.jpeg","./asset/Mai3.jpeg","./asset/Mai4.jpeg","./asset/Mai5.jpeg","./asset/Mai6.jpeg","./asset/Mai7.jpeg","./asset/Mai8.jpeg","./asset/Mai9.jpeg"
];
let sunSlideshowInterval = null;
let lastIndex = -1;
function getRandomIndex() {
  let idx;
  do { idx = Math.floor(Math.random() * selfieImages.length); } while (idx === lastIndex);
  lastIndex = idx;
  return idx;
}
function preloadSelfies() {
  return Promise.all(selfieImages.map(src => new Promise(res => { const img = new Image(); img.onload = res; img.src = src; })));
}
...
function animateLoaderBall() {
  ...
  const minSize = 24, maxSize = 500;
  function updateBall() {
    percent.textContent = current + "%";
    const size = minSize + (maxSize - minSize) * (current / 100);
    ball.style.width = size + "px";
    ball.style.height = size + "px";
    ball.style.transition = "width 0.2s linear, height 0.2s linear, font-size 0.2s linear";
    ball.style.fontSize = 0.7 + 1.3 * (current / 100) + "em";
  }
  const totalDuration = 2000, steps = 100, intervalTime = totalDuration / steps;
  const interval = setInterval(() => {
    ...
    if (current >= 100) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          gsap.to([nav, smile], { opacity: 1, y: 0, duration: 1, ease: "power2.out" });
...
function setupGSAPAnimations() {
  ...
  mainScrollTimeline.to(".text-approach-animation", {
  ...
  skillsTimeline.fromTo(".designSkills", getSlideTween(".designSkills", 50).from, getSlideTween(".designSkills", 50).to);
  skillsTimeline.fromTo(".devSkills", getSlideTween(".devSkills", 60).from, getSlideTween(".devSkills", 60).to, "<0.5");
  skillsTimeline.fromTo(".softSkills", getSlideTween(".softSkills", 70).from, getSlideTween(".softSkills", 70).to, "<0.5");
}
// --- Main Init ---
